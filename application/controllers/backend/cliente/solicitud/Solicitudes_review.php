...
class Solicitudes_review extends MY_Controller{
	...
	function __construct(){
	 ...
	 function nueva_solicitud($id){

		
 $cliente=$this->session->userdata('s_idusuario');
...
$idsol="";
		...
		echo json_encode($data['observaciones']);

		//$data['observaciones_sol'] = $this->mObservacion->consulta_obs_campo($idsol);
		$data['seccion']= $this->mSeccion->consulta();
		$data['norma_comp']= $this->mNorma->consulta();
		$this->load->view('backend/cliente/solicitud/solicitud_review',$data);
			$this->load->view('backend/cliente/observaciones_c',$data);
			
		$this->load->view('backend/cliente/templates/pre_footer');
		$this->load->view('backend/cliente/templates/footer',$data);
